...
private:
    ...
    void add_vertical_loop(int other_col) {
        vector<int> belt;
        for (int r = 0; r < N; r++) belt.push_back(cell_id(r, EXIT_COL));
        for (int r = N - 1; r >= 0; r--) belt.push_back(cell_id(r, other_col));
    ...
    double heuristic(const State &state) const {
        static const array<double, 5> weight = {0.50, 0.24, 0.12, 0.06, 0.03};
        double value = 0.0;
    ...
    void build_plans_for_path(const vector<int> &path, int stage, int target,
        ...
        vector<pair<int, int>> options = rotation_options(belt, current_cell, goals, 4);
    ...
    vector<Operation> choose_next_ops(const State &state) const {
        const int WIDTH = 4;
        const int DEPTH = 2;
        vector<Candidate> first = current_box_plans(state, 10);
        ...
        for (int depth = 1; depth < DEPTH; depth++) {
            ...
            for (const auto &node : beam) {
                ...
                vector<Candidate> plans = current_box_plans(node.state, 6);
...
int main() {
    ...
    for (int side : {-1, 1}) {
        for (bool helper : {false, true}) {
            configs.push_back({side, helper, false, false});
        }
    }
