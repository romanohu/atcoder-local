...
public:
    ...
    Candidate solve() {
        if (config.layout == 1) return solve_global_simple();
...
private:
    ...
    void add_vertical_loop_pair(int c0, int c1) {
        vector<int> belt;
    ...
    void add_vertical_loop(int other_col) {
        add_vertical_loop_pair(EXIT_COL, other_col);
    }

    void add_global_snake_loop() {
        vector<int> belt;
        ...
        for (int r = N - 1; r >= 1; r--) {
            if (r % 2 == 1) {
                for (int c = N - 2; c >= 0; c--) belt.push_back(cell_id(r, c));
            } else {
                for (int c = 0; c <= N - 2; c++) belt.push_back(cell_id(r, c));
            }
        }
    ...
    void build_belts() {
        ...
        if (config.layout == 1) {
            ...
            add_global_snake_loop();
        } else if (config.layout == 2) {
            for (int r = 0; r < N; r += 2) add_horizontal_loop(r, 0, N - 1);
            for (int c = 0; c < N; c += 2) add_vertical_loop_pair(c, c + 1);
    ...
    void build_indexes() {
        ...
        if (config.layout != 1) build_route_cache();
    }
    ...
    Candidate solve_global_simple() const {
        State state;
        state.board = initial;
        state.pos.assign(TOTAL, -1);
        for (int id = 0; id < TOTAL; id++) {
        ...
        int global_belt = -1;
        for (int b = 0; b < (int)belts.size(); b++) {
            if ((int)belts[b].size() == TOTAL) global_belt = b;
        }

        vector<Operation> ops;
        while (state.next_box < TOTAL && (int)ops.size() < MAX_OPS) {
            int target = state.next_box;
            int cur = belt_index[global_belt][state.pos[target]];
            int goal = belt_index[global_belt][EXIT_ID];
            int len = belts[global_belt].size();
            int plus = (goal - cur + len) % len;
            int minus = (cur - goal + len) % len;
            int dir = (plus <= minus ? 1 : -1);
            int steps = min(plus, minus);

            for (int i = 0; i < steps && state.next_box == target; i++) {
                rotate_belt(state, global_belt, dir, &ops);
                if ((int)ops.size() >= MAX_OPS) break;
            }

            if (steps == 0 && state.next_box == target) break;
        }

        Candidate result;
        result.ops = ops;
        result.state = state;
        result.score = (state.next_box == TOTAL ? ops.size()
                                                : 1000000.0 + (TOTAL - state.next_box) * 1000.0);
        return result;
    }
    ...
    vector<vector<int>> compute_paths(int start_cell) const {
        ...
        for (int b : starts) {
            dfs(dfs, b, 0);
            if ((int)paths.size() >= 48) break;
        }
        sort(paths.begin(), paths.end());
        paths.erase(unique(paths.begin(), paths.end()), paths.end());
        if ((int)paths.size() > 48) paths.resize(48);
        return paths;
    }
    ...
    vector<Operation> choose_next_ops(const State &state) const {
        vector<Candidate> first = current_box_plans(state, config.first_limit);
        ...
        auto trim = [&]() {
            ...
            if ((int)beam.size() > config.width) beam.resize(config.width);
        };
        trim();
        ...
        for (int depth = 1; depth < config.depth; depth++) {
            ...
            for (const auto &node : beam) {
                ...
                vector<Candidate> plans = current_box_plans(node.state, config.next_limit);
...
int main() {
    ...
    for (int side : {-1, 1}) {
        for (bool helper : {false, true}) {
            configs.push_back({0, side, helper, false, false, 10, 3, 20, 10});
        }
    }
    configs.push_back({1, 1, false, false, false, 4, 2, 8, 5});
