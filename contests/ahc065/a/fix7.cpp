...
public:
    ...
    Candidate solve() {
#ifdef FORCE_OPERATION_BEAM
        return solve_operation_beam();
#elif defined(USE_HYBRID_OPERATION_BEAM)
        return solve_hybrid_beam();
#else
        return solve_box_beam();
#endif
    }
...
private:
    ...
    void add_ranked_action(vector<pair<int, Operation>> &actions, int priority,
                           int belt, int dir) const {
        if (belt < 0 || belt >= (int)belts.size()) return;
        actions.push_back({priority, {belt, dir}});
    }

    vector<Operation> candidate_actions(const State &state) const {
        vector<pair<int, Operation>> ranked;
        int target = state.next_box;

        for (int belt : exit_belts) {
            add_ranked_action(ranked, 0, belt, 1);
            add_ranked_action(ranked, 1, belt, -1);
        }
        ...
        for (int offset = 0; offset < 10; offset++) {
            ...
            for (int belt : cell_belts[cell]) {
                int priority = 20 + offset * 10;
                add_ranked_action(ranked, priority, belt, 1);
                add_ranked_action(ranked, priority + 1, belt, -1);
            }

            for (int pi = 0; pi < (int)paths.size() && pi < 3; pi++) {
                ...
                if ((int)path.size() == 1) {
                    ...
                    add_ranked_action(ranked, 35 + offset * 12 + pi * 4, path[1], 1);
                    add_ranked_action(ranked, 36 + offset * 12 + pi * 4, path[1], -1);
                }
                ...
                for (auto [steps, dir] : opts) {
                    int slack = (offset == 0 ? 6 : 4);
                    if (steps > 0 && steps <= best + slack) {
                        int priority = offset * 80 + pi * 12 + steps;
                        add_ranked_action(ranked, priority, belt, dir);
                    }
                }
            }
        }
        ...
        sort(ranked.begin(), ranked.end(), [](const auto &a, const auto &b) {
            ...
            if (a.second.belt != b.second.belt) return a.second.belt < b.second.belt;
            return a.second.dir < b.second.dir;
        });

        vector<Operation> actions;
        vector<array<int, 2>> used(belts.size(), {0, 0});
        for (const auto &[priority, op] : ranked) {
            (void)priority;
            int idx = (op.dir == 1 ? 1 : 0);
            if (used[op.belt][idx]) continue;
            used[op.belt][idx] = 1;
            actions.push_back(op);
            if ((int)actions.size() >= 18) break;
        }
    ...
    double operation_score(const State &state, int start_next, int cost) const {
        ...
        return -50000.0 * done + cost + 12.0 * heuristic(state);
    }
    ...
    Candidate solve_operation_beam() {
        ...
        while (state.next_box < TOTAL && (int)all_ops.size() < MAX_OPS) {
            ...
            if (state.next_box == before) {
                no_progress_ops += chunk.size();
                if (no_progress_ops > 700) break;
            } else {
                no_progress_ops = 0;
            }
        }
    ...
    State apply_ops_to_copy(const State &state, const vector<Operation> &ops) const {
        State next_state = state;
        for (Operation op : ops) rotate_belt(next_state, op.belt, op.dir, nullptr);
        return next_state;
    }

    bool should_try_operation_beam(const State &state) const {
        if (state.next_box >= TOTAL) return false;
        int dist = estimate_box_distance(state, state.next_box);
        if (dist <= 12) return true;
        return exit_queue_bonus(state) >= 12.0;
    }

    bool operation_chunk_is_better(const State &before,
                                   const vector<Operation> &box_ops,
                                   const State &box_state,
                                   const vector<Operation> &op_ops,
                                   const State &op_state) const {
        int box_done = box_state.next_box - before.next_box;
        int op_done = op_state.next_box - before.next_box;
        if (op_done <= 0) return false;
        if (op_done < box_done) return false;
        if (op_done == box_done) {
            double box_score = box_ops.size() + heuristic(box_state);
            double op_score = op_ops.size() + heuristic(op_state);
            return op_score + 2.0 < box_score;
        }

        State standard_state = box_state;
        int standard_ops = box_ops.size();
        while (standard_state.next_box < op_state.next_box &&
               standard_ops <= (int)op_ops.size()) {
            vector<Operation> extra_ops = choose_next_ops(standard_state);
            if (extra_ops.empty()) break;
            standard_ops += extra_ops.size();
            standard_state = apply_ops_to_copy(standard_state, extra_ops);
        }

        if (standard_state.next_box >= op_state.next_box) {
            double standard_score = standard_ops + heuristic(standard_state);
            double op_score = op_ops.size() + heuristic(op_state);
            return op_score + 2.0 < standard_score;
        }
        return (int)op_ops.size() <= standard_ops;
    }

    Candidate solve_hybrid_beam() {
        State state;
        state.board = initial;
        state.pos.assign(TOTAL, -1);
        for (int id = 0; id < TOTAL; id++) {
        ...
        vector<Operation> all_ops;
        while (state.next_box < TOTAL && (int)all_ops.size() < MAX_OPS) {
            vector<Operation> box_ops = choose_next_ops(state);
            if (box_ops.empty()) break;

            State box_state = apply_ops_to_copy(state, box_ops);
            vector<Operation> chosen_ops = box_ops;

            if (should_try_operation_beam(state)) {
                vector<Operation> op_ops = choose_operation_chunk(state);
                if (!op_ops.empty()) {
                    State op_state = apply_ops_to_copy(state, op_ops);
                    if (operation_chunk_is_better(state, box_ops, box_state, op_ops, op_state)) {
                        chosen_ops = op_ops;
                    }
                }
            }

            for (Operation op : chosen_ops) {
                if ((int)all_ops.size() >= MAX_OPS) break;
                rotate_belt(state, op.belt, op.dir, nullptr);
                all_ops.push_back(op);
                if (state.next_box == TOTAL) break;
