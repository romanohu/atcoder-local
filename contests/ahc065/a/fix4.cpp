...
private:
    ...
    vector<vector<int>> compute_grid_paths(int start_cell) const {
        ...
        int h = start_cell / N / 2;
        int v = 10 + (start_cell % N) / 2;
        int exit_h = 0;
        int exit_v = 10 + EXIT_COL / 2;

        if (belt_index[h][EXIT_ID] != -1) paths.push_back({h});
        if (belt_index[v][EXIT_ID] != -1) paths.push_back({v});
        if (h != exit_v) paths.push_back({h, exit_v});
        if (v != exit_h) paths.push_back({v, exit_h});
    ...
    vector<Operation> choose_next_ops(const State &state) const {
        const int WIDTH = 10;
        const int DEPTH = 3;
        vector<Candidate> first = current_box_plans(state, 20);
        ...
        for (int depth = 1; depth < DEPTH; depth++) {
            ...
            for (const auto &node : beam) {
                ...
                vector<Candidate> plans = current_box_plans(node.state, 10);
...
int main() {
    ...
    configs.push_back({1, 1, false, false, false});
