...
#ifndef BATCH_WINDOW
#define BATCH_WINDOW 12
#endif

#ifndef BATCH_QUEUE_WEIGHT
#define BATCH_QUEUE_WEIGHT 0
#endif

#ifndef FLOW_QUEUE_WEIGHT
#define FLOW_QUEUE_WEIGHT 0
#endif

#ifndef LIGHT_BEAM_DEPTH
#define LIGHT_BEAM_DEPTH 2
#endif

#ifndef LIGHT_BEAM_WIDTH
#define LIGHT_BEAM_WIDTH 6
#endif
...
private:
    ...
    void build_belts() {
        ...
        if (config.layout == 1) {
            ...
            for (int r = 0; r < N; r += 2) {
                add_horizontal_loop(r, 0, EXIT_COL - 1);
                add_horizontal_loop(r, EXIT_COL, N - 1);
            }
            for (int c = 1; c + 1 < N; c += 2) add_vertical_pair_loop(c, c + 1);
    ...
    int move_cost_to_cell_on_any_belt(int cell, int goal_cell) const {
        int best = numeric_limits<int>::max();
        for (int belt : cell_belts[cell]) {
            if (belt_index[belt][goal_cell] == -1) continue;
            best = min(best, min_distance_between(belt, cell, {goal_cell}));
        }
        return best;
    }

    double batch_queue_bonus(const State &state) const {
        double best_score = 0.0;

        for (int exit_belt : exit_belts) {
            for (int dir : {-1, 1}) {
                int prev_steps = -1;
                int prefix = 0;
                double score = 0.0;

                for (int offset = 0; offset < BATCH_WINDOW; offset++) {
                    ...
                    if (belt_index[exit_belt][cell] != -1) {
                        int steps = steps_to_exit_on_belt(exit_belt, cell, dir);
                        if (offset == 0 || steps > prev_steps) {
                            int gap = (offset == 0 ? steps : steps - prev_steps);
                            double cost = 0.18 * steps + 0.35 * max(0, gap - 5);
                            best_cost = cost;
                            best_steps = steps;
                        }
                    }

                    for (int goal_cell : belts[exit_belt]) {
                        int steps = steps_to_exit_on_belt(exit_belt, goal_cell, dir);
                        if (offset > 0 && steps <= prev_steps) continue;
                        int move_cost = move_cost_to_cell_on_any_belt(cell, goal_cell);
                        if (move_cost == numeric_limits<int>::max()) continue;

                        int gap = (offset == 0 ? steps : steps - prev_steps);
                        double cost = move_cost + 0.12 * steps + 0.25 * max(0, gap - 6);
                        if (cost < best_cost) {
                            best_cost = cost;
                            best_steps = steps;
                        }
                    ...
                    if (best_steps == numeric_limits<int>::max()) break;

                    double limit = 16.0 + offset * 1.6;
                    if (best_cost > limit) break;

                    double order_weight = BATCH_WINDOW - offset;
                    score += order_weight * max(0.0, limit - best_cost);
                    prev_steps = best_steps;
                    prefix++;
                }

                score += prefix * prefix * 8.0;
    ...
    double flow_queue_bonus(const State &state) const {
        double best = 0.0;
        ...
        for (int belt : exit_belts) {
            for (int dir : {-1, 1}) {
                ...
                for (int offset = 0; offset < 18; offset++) {
                    ...
                    int steps = steps_to_exit_on_belt(belt, cell, dir);
                    if (offset > 0 && steps <= prev_steps) break;

                    int gap = (offset == 0 ? steps : steps - prev_steps);
                    if (gap > 24) break;

                    score += (18 - offset) * max(0, 24 - gap);
                    prev_steps = steps;
                    prefix++;
                }

                score += prefix * prefix * 24.0;
                if (prev_steps >= 0) score -= prev_steps * 0.35;
                best = max(best, score);
            }
        }
    ...
    double search_score(const State &state, int fixed_cost) const {
#if FLOW_QUEUE_WEIGHT == 0
        double score = rollout_score(state, fixed_cost);
#else
        double score = rollout_score(state, fixed_cost)
                       - (FLOW_QUEUE_WEIGHT / 100.0) * flow_queue_bonus(state);
#endif
#if BATCH_QUEUE_WEIGHT == 0
        return score;
#else
        return score - (BATCH_QUEUE_WEIGHT / 100.0) * batch_queue_bonus(state);
#endif
    }
    ...
    vector<Operation> choose_next_ops(const State &state) const {
        ...
        int width = (light_grid ? LIGHT_BEAM_WIDTH : 10);
        int depth_limit = (light_grid ? LIGHT_BEAM_DEPTH : 3);
        ...
        auto trim = [&]() {
            ...
            for (Node &node : beam) {
                node.score = search_score(node.state, node.cost);
            }
            sort(beam.begin(), beam.end(), [](const Node &a, const Node &b) {
                if (a.score != b.score) return a.score < b.score;
                return a.cost < b.cost;
...
#ifdef ONLY_LAYOUT6
    configs.push_back({6, 1, false, false, false});
#elif defined(TRY_LAYOUT6)
