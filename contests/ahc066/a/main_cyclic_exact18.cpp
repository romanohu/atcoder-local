...
#define HIER_CAND_LIMIT 1600
#define HIER_MAX_LEN 160
#define HIER_START_LIMIT 120
#define HIER_S_PRIORITY_WEIGHT 1
#define SOLVER_TIME_LIMIT_MS 1450
...
int main() {
    ...
    auto make_exact_small_order = [&]() -> vector<int> {
        if (M < 10 || M > 18) return {};
        const int full = 1 << M;
        const int inf = 1000000000;
        const int states = full * M * 4;
        vector<int> dp(states, inf);
        vector<unsigned char> prev_item(states, 255);
        vector<unsigned char> prev_dir(states, 255);
        auto idx_of = [&](int mask, int last, int end_dir) {
            return ((mask * M + last) << 2) | end_dir;
        };

        for (int k = 0; k < M; ++k) {
            int ball_node = 1 + k;
            int basket_node = 1 + M + k;
            const Route &to_ball = route[0][1][ball_node];
            if (to_ball.end_dir == -1) continue;
            const Route &to_basket =
                route[ball_node][to_ball.end_dir][basket_node];
            if (to_basket.end_dir == -1) continue;
            int end_dir = to_basket.end_dir;
            int idx = idx_of(1 << k, k, end_dir);
            dp[idx] = to_ball.cost + 1 + to_basket.cost + 1;
        }

        for (int mask = 1; mask < full && !order_time_over(); ++mask) {
            for (int last = 0; last < M; ++last) {
                if (((mask >> last) & 1) == 0) continue;
                int current_node = 1 + M + last;
                for (int dir = 0; dir < 4; ++dir) {
                    int cur_idx = idx_of(mask, last, dir);
                    int cur_cost = dp[cur_idx];
                    if (cur_cost == inf) continue;
                    int remaining = (~mask) & (full - 1);
                    while (remaining) {
                        int bit = remaining & -remaining;
                        int k = __builtin_ctz(bit);
                        remaining -= bit;
                        int ball_node = 1 + k;
                        int basket_node = 1 + M + k;
                        const Route &to_ball = route[current_node][dir][ball_node];
                        if (to_ball.end_dir == -1) continue;
                        const Route &to_basket =
                            route[ball_node][to_ball.end_dir][basket_node];
                        if (to_basket.end_dir == -1) continue;
                        int next_mask = mask | bit;
                        int end_dir = to_basket.end_dir;
                        int next_idx = idx_of(next_mask, k, end_dir);
                        int next_cost =
                            cur_cost + to_ball.cost + 1 + to_basket.cost + 1;
                        if (next_cost < dp[next_idx]) {
                            dp[next_idx] = next_cost;
                            prev_item[next_idx] = static_cast<unsigned char>(last);
                            prev_dir[next_idx] = static_cast<unsigned char>(dir);
                        }
                    }
                }
            }
        }
        if (order_time_over()) return {};
        ...
        for (int last = 0; last < M; ++last) {
            for (int dir = 0; dir < 4; ++dir) {
                int idx = idx_of(full_mask, last, dir);
                if (dp[idx] < best_cost) {
                    best_cost = dp[idx];
                    best_idx = idx;
                    best_last = last;
        ...
        while (mask) {
            ...
            int idx = idx_of(mask, last, dir);
            int p_item = prev_item[idx];
            int p_dir = prev_dir[idx];
            mask ^= 1 << last;
            if (mask == 0) break;
            last = p_item;
            dir = p_dir;
    ...
    add_candidate(make_exact_small_order(), 0);
