...
`uv sync --group dev` は `online-judge-tools`、`oj-bundle`、`aclogin` とテスト用の `pytest` を仮想環境へインストールします。`atcoder-cli` は npm のグローバルパッケージとしてインストールします。`uv tool install .` の後は、新しいシェルでもすぐに `acc` を使えます。インストール済みの `acc` 実行ファイル自体はインストール時のチェックアウトの絶対パスに依存しないため、後からリポジトリを移動しても使用できます。ただし、次節で設定するテンプレートの絶対パスは別途更新が必要です。

`git pull` などでラッパーのソースコードを更新した後、またはパッケージのメタデータや依存関係を更新した後は、次を実行してインストール済みコマンドを更新します。
...
リポジトリルートで次を実行します。テンプレートの設定値には、このクローンの絶対パスが保存されます。リポジトリを移動した場合は、移動後のリポジトリルートでこの設定をもう一度実行してテンプレートのパスを更新してください。
